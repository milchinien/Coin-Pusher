/* =========================================================================
   tree.ts — Generischer Skill-Tree. Uebernommen aus Dropfall.
   ...
   Die Farbe kommt vom Ast — im Reliktenschieber gibt es nur eine Waehrung,
   die Farbe muss also nichts ueber den Preis sagen. Was man sich gerade
   nicht leisten kann, verliert Saettigung — siehe `klamm()`.
...
/** Womit ein Knoten bezahlt wird. Im Reliktenschieber nur Geld. */
export type TreeCurrency = "money";
...
export class TreeView {
  ...
  /** Welchen Palettenslot ein Knoten traegt: den seines Astes. */
  ...
  /** Das Piktogramm eines Knotens: ein Glyph, weiss auf der Deckflaeche. */
  private icon(
    ctx: CanvasRenderingContext2D,
    def: TreeNodeDef,
    cx: number,
    ...
    this.glyph(ctx, def.icon, cx, cy, size * (def.icon.length > 1 ? 0.5 : 0.72), color);
  }
